...
set OSB_HOME=Package\Oracle\Middleware\Oracle_OSB1
set WL_HOME=Package\Oracle\Middleware\wlserver_10.3
set JAVA_HOME=Package\Program
...
@rem set CONFIGJAR_HOME=%OSB_HOME%\tools\configjar

@rem set CONFIGJAR_HOME=Package
...
set CLASSPATH=%CLASSPATH%;configjar.jar
set CLASSPATH=%CLASSPATH%;L10N

cmd.exe /C "Package\Oracle\Middleware\modules\org.apache.ant_1.7.1\bin\ant.bat -file build.xml run && exit %%ERRORLEVEL%% 


cmd.exe /C "Package\Oracle\Middleware\modules\org.apache.ant_1.7.1\bin\ant.bat -file build.xml importToOSB && exit %%ERRORLEVEL%% 
