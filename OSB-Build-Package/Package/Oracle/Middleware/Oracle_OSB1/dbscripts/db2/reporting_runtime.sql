...
CREATE TABLE WLI_QS_REPORT_ATTRIBUTE (
  ...
  DB_TIMESTAMP          TIMESTAMP    DEFAULT CURRENT TIMESTAMP NOT NULL,
  ...
  CONSTRAINT PK_WLI_QS_RPT_ATTR PRIMARY KEY(MSG_GUID)
);

CREATE INDEX IX_WLI_QS_RPT_AT_D ON WLI_QS_REPORT_ATTRIBUTE( DB_TIMESTAMP, MSG_LABELS );

CREATE INDEX IX_WLI_QS_RPT_AT_I ON WLI_QS_REPORT_ATTRIBUTE( INBOUND_SERVICE_NAME, ERROR_CODE, DB_TIMESTAMP);
...
CREATE TABLE WLI_QS_REPORT_DATA(
  ...
  ENCODING    VARCHAR(24),
  DATA_VALUE  BLOB(1G),
  CONSTRAINT FK_WLI_QS_RPT_DATA FOREIGN KEY(MSG_GUID)
    REFERENCES WLI_QS_REPORT_ATTRIBUTE(MSG_GUID) ON DELETE CASCADE
);   

CREATE UNIQUE INDEX IX_WLI_QS_RPT_DATA ON WLI_QS_REPORT_DATA(MSG_GUID);