...
function  WWHBrowser_Initialize()
{
  ...
  var  Version = 0.0;
   ...
   // Reset locale to correct language value; the locale string is already present in help url.
   // extract it from url.
    var mLocaleTempStr;

    var windowUrl = document.location.href;

    var webAppHelpSearchStr = "sbconsole/help/";   // locale string is present after this in url

    // find out the locale string between  "sbconsole/help/" and "/" in url
    var helpDirIdx = windowUrl.indexOf(webAppHelpSearchStr);
    if(helpDirIdx != -1) {
        mLocaleTempStr = windowUrl.substring(helpDirIdx+ webAppHelpSearchStr.length);
        if(mLocaleTempStr.indexOf("/") != -1){
            mLocaleTempStr = mLocaleTempStr.substring(0, mLocaleTempStr.indexOf("/"));
        } else {
            mLocaleTempStr = "undefined";    
        }
    }

    if(mLocaleTempStr == "undefined") {
