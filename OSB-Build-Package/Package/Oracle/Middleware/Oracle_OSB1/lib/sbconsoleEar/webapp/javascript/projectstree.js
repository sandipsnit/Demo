...
// This variable is used to find the context of jsp page.
var TREE_CONTEXT_PAGE = null;
...
function mouseOverCall(nodeId, popupShowId)
{
	...
	var element = getLinkElement(node.nId);
  	...
  	setMouseOverOutBackGroundColor(element);
...
function setMouseOverOutBackGroundColor(element){
    if(isTreeContextPagePipeline()) {
        element.style.backgroundColor = '#FFFFFF';
    } else {
...
function isTreeContextPagePipeline(){
     if(TREE_CONTEXT_PAGE != null && TREE_CONTEXT_PAGE == 'PIPELINE') {
         return true;
     } else {
         return false;
     }
...
function createTree(startNode)
{
  ...
  // Auto scroll is implemented in functions.js
...
function writeNode(node, icon, highlighted, className, accessibility )
{
   ...
   if(node._prefix != undefined)
    ...
    var isPipeline = isTreeContextPagePipeline();

    if(ish)
    {
        var hilitBgcolor;
        if(isPipeline) {
            hilitBgcolor = 'BACKGROUND-COLOR: #E0E0E0';
        } else {
            hilitBgcolor = 'BACKGROUND-COLOR: #FFFFFF';
        }
        document.write('<span style="'+hilitBgcolor+'">');
    }
    if(!isPipeline) document.write("&nbsp;");
...
/*
This function adds scroll positionto the url
*/
function addScrollPositionsToUrl(url){
    ...
    url = addUrlParameter(url, "treeScrollXPosition", x);
    url = addUrlParameter(url, "treeScrollYPosition", y);
    return url;
}

function setScrollPostions(){
    var x = 0; var y = 0;
    ...
    document.HierarchyManagementForm.treeScrollXPosition.value = x;
    document.HierarchyManagementForm.treeScrollYPosition.value = y;
}
