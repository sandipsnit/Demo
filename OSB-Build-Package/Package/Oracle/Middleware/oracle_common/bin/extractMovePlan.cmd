...
@REM extractMovePlan.cmd
@REM
@REM Copyright (c) 2010, 2013, Oracle and/or its affiliates. All rights reserved. 
@REM
@REM    NAME
@REM      extractPlans.cmd - Script to extract t2p move plan
@REM
@REM    DESCRIPTION
@REM      This script is used to extract t2p move plan along with other J2EE configuration plans. 
...
:invokeCloningClient
set ERRORLEVEL=
"%JRE_DIR%\bin\java.exe" %T2P_JAVA_OPTIONS% -jar "%COMMON_HOME%\jlib\cloningclient.jar" applyClone -script extractMovePlan -emp true %ARGS%
...
:help
echo usage: extractMovePlan.cmd -javaHome java_home -archiveLoc archive_location -planDirLoc dir_location
echo        [-optimizationHints optimization_hints] [-logDirLoc log_directory] [-silent true^|false]
echo.
echo Try "extractMovePlan.cmd -javaHome java_home  -help"  for more information
goto end
