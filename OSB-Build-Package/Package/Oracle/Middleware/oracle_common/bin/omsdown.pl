#!/usr/local/bin/perl
#
# $Header: omsdown.pl 26-apr-2005.15:05:32 aholser Exp $
#
# omsstop.pl
#
...
#
#    DESCRIPTION
#      Mail specified user that oms is down - abnormally terminated
#
#    NOTES
#      can be invoked stand-alone for testing purposes
#
#    MODIFIED   (MM/DD/YY)
#    aholser     04/26/05 - 
#    aholser     05/11/04 - 
...
#
  ...
  system ("echo Entry");
  my $fn = "$ENV{ORACLE_HOME}/sysman/config/omsdown";
  system "date > $fn";
  if(!open(FH, ">>$fn")) 
  {
  ...
  my $subject="Enterprise Manager oms has terminated abnormally";
  print FH "The Management Service in ORACLE_HOME $ENV{ORACLE_HOME} has terminated abnormally.\n";
  ...
  my $command1="cat $filename | grep 'em_oob_crash=' | grep -v '#' | sed s/em_oob_crash=// | awk '{print $1}'";
  my $isok=`$command1` or system("echo em_oob_crash not defined");
  ...
  $command1="cat $filename | grep 'em_email_address=' | grep -v '#' | sed s/em_email_address=// | awk '{print $1}'";
