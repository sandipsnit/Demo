...
--
-- MODIFIED    (MM/DD/YY)
-- jhsi         09/16/11   - Added MDS_METADATA_DOCS table and
--                           PATH_CONTENT_TYPE column
-- erwang       08/23/11   - Added PATH_DOC_MOTYPE_NAME and 
--                           PATH_DOC_MOETYPE_NSURI
-- jhsi         08/16/11   - Added PARTITION_MT_STATE column
-- rchalava     08/16/11   - Reverted purge path table changes.
-- rchalava     07/20/11   - Repos mgmt MT support
...
BEGIN
  ...
  CNT := 0;
  SELECT COUNT(*) INTO CNT FROM USER_TABLES WHERE TABLE_NAME = 'MDS_PARTITIONS';

  IF (CNT > 0) THEN
    EXECUTE IMMEDIATE 'drop table MDS_PARTITIONS';
  END IF;

  CNT := 0;
  SELECT COUNT(*) INTO CNT FROM USER_TABLES WHERE TABLE_NAME = 'MDS_PATHS';

  IF (CNT > 0) THEN
    EXECUTE IMMEDIATE 'drop table MDS_PATHS';
  END IF;

  CNT := 0;
  SELECT COUNT(*) INTO CNT FROM USER_TABLES WHERE TABLE_NAME = 'MDS_STREAMED_DOCS';

  IF (CNT > 0) THEN
    EXECUTE IMMEDIATE 'drop table MDS_STREAMED_DOCS';
  END IF;
  ...
  IF (CNT > 0) THEN
    EXECUTE IMMEDIATE 'drop table MDS_METADATA_DOCS';
  END IF;

  CNT := 0;
  SELECT COUNT(*) INTO CNT FROM USER_TABLES WHERE TABLE_NAME = 'MDS_DEPENDENCIES';

  IF (CNT > 0) THEN
    EXECUTE IMMEDIATE 'drop table MDS_DEPENDENCIES';
  END IF;

  CNT := 0;
  SELECT COUNT(*) INTO CNT FROM USER_TABLES WHERE TABLE_NAME = 'MDS_TXN_LOCKS';

  IF (CNT > 0) THEN
    EXECUTE IMMEDIATE 'drop table MDS_TXN_LOCKS';
  END IF;

  CNT := 0;
  SELECT COUNT(*) INTO CNT FROM USER_TABLES WHERE TABLE_NAME = 'MDS_LABELS';

  IF (CNT > 0) THEN
    EXECUTE IMMEDIATE 'drop table MDS_LABELS';
  END IF;

  CNT := 0;
  SELECT COUNT(*) INTO CNT FROM USER_TABLES WHERE TABLE_NAME = 'MDS_TRANSACTIONS';

  IF (CNT > 0) THEN
    EXECUTE IMMEDIATE 'drop table MDS_TRANSACTIONS';
  END IF;
  ...
  IF (CNT > 0) THEN
    EXECUTE IMMEDIATE 'drop table MDS_DEPL_LINEAGES';
  END IF;

  CNT := 0;
  SELECT COUNT(*) INTO CNT FROM USER_TABLES WHERE TABLE_NAME = 'MDS_SANDBOXES';

  IF (CNT > 0) THEN
    EXECUTE IMMEDIATE 'drop table MDS_SANDBOXES';
  END IF;

  CNT := 0;
  SELECT COUNT(*) INTO CNT FROM USER_TABLES WHERE TABLE_NAME = 'MDS_PURGE_PATHS';

  IF (CNT > 0) THEN
    EXECUTE IMMEDIATE 'drop table MDS_PURGE_PATHS';
  END IF;
END;
/
...
create global temporary table MDS_PURGE_PATHS (
