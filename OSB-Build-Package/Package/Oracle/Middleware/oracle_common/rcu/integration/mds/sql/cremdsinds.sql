...
--    under the oracleebr folder for creating indexes on the EBR-enabled
--    schema tables.
-- 
-- MODIFIED    (MM/DD/YY)
...
-- erwang       04/18/11   - #(10360031) Added MDS_PATHS_N8
...
on MDS_PATHS( PATH_OWNER_DOCID, PATH_NAME, PATH_VERSION, PATH_PARTITION_ID, ENTERPRISE_ID, PATH_GUID )
;
...
on MDS_PATHS( PATH_FULLNAME, PATH_VERSION, PATH_PARTITION_ID, ENTERPRISE_ID, PATH_GUID )
;

REM Find documents by name
REM drop index MDS_PATHS_N1; 
create index MDS_PATHS_N1
on MDS_PATHS( PATH_NAME, PATH_PARTITION_ID, ENTERPRISE_ID, PATH_TYPE, PATH_CONTENTID, PATH_DOC_ELEM_NSURI, PATH_DOC_ELEM_NAME, PATH_GUID )
;
...
create index MDS_PATHS_N2 on MDS_PATHS( PATH_LOW_CN, PATH_PARTITION_ID )
;

REM #(5690942) - Find documents by highCN
REM drop index MDS_PATHS_N3; 
create index MDS_PATHS_N3 on MDS_PATHS( PATH_HIGH_CN, PATH_PARTITION_ID )
;

REM #(5840250) - Find documents by fullname.
create index MDS_PATHS_N4 on MDS_PATHS( PATH_FULLNAME, PATH_HIGH_CN, ENTERPRISE_ID, PATH_PARTITION_ID )
;

REM #(5998529) - Find documents by GUID
create index MDS_PATHS_N5 on MDS_PATHS( PATH_PARTITION_ID, ENTERPRISE_ID, PATH_TYPE, PATH_GUID, PATH_HIGH_CN, PATH_LINEAGE_ID )
;
...
create INDEX MDS_PATHS_N6 on MDS_PATHS 
...
create index MDS_PATHS_N7 on MDS_PATHS
...
create index MDS_PATHS_N8 on MDS_PATHS
	( PATH_CONTENTID, PATH_PARTITION_ID )
;

REM #(12859728) index for root element name (MOType) for query.
create index MDS_PATHS_N9 on MDS_PATHS
        ( PATH_DOC_ELEM_NAME, PATH_PARTITION_ID )
;

REM #(6838583) - Indexes for purge
create index MDS_PURGE_PATHS_N1 on MDS_PURGE_PATHS
  ( PPATH_CONTENTID, PPATH_PARTITION_ID )
;

create index MDS_PURGE_PATHS_N2 on MDS_PURGE_PATHS
...
create unique index MDS_ATTRIBUTES_U2 on MDS_ATTRIBUTES ( ATT_LOCALNAME, ATT_CONTENTID, ATT_PARTITION_ID, ATT_COMP_SEQ, ATT_NSID )
; 
...
on MDS_DEPENDENCIES ( DEP_PARENT_DOCNAME, DEP_PARENT_LOCALREF, DEP_HIGH_CN, DEP_PARTITION_ID, ENTERPRISE_ID, DEP_TOCHILD_ROLE )
;
...
on MDS_DEPENDENCIES( DEP_CHILD_DOCID, DEP_CHILD_LOCALREF, DEP_HIGH_CN,
DEP_PARTITION_ID )
;
...
on MDS_LABELS ( LABEL_PARTITION_ID, ENTERPRISE_ID, LABEL_NAME )
;


REM #(5690942) - Find labels by CN
REM drop index MDS_LABELS_N1; 
create index MDS_LABELS_N1 on MDS_LABELS( LABEL_CN, LABEL_PARTITION_ID )
;
...
on MDS_SANDBOXES ( SB_PARTITION_ID, ENTERPRISE_ID, SB_NAME )
;
...
on MDS_LARGE_ATTRIBUTES( ATT_CONTENTID, ATT_COMP_SEQ, ATT_PARTITION_ID, ATT_SEQ )
;

REM Find attribute by name
REM drop index MDS_LARGE_ATTRIBUTES_N1;
create index MDS_LARGE_ATTRIBUTES_N1
on MDS_LARGE_ATTRIBUTES ( ATT_LOCALNAME, ATT_CONTENTID, ATT_PARTITION_ID )
; 


REM Unique constraints
create unique index MDS_DEPL_LINEAGE_U1 
on MDS_DEPL_LINEAGES ( DL_LINEAGE_ID, ENTERPRISE_ID, DL_PARTITION_ID )
;

create unique index MDS_DEPL_LINEAGE_U2 
on MDS_DEPL_LINEAGES ( DL_APPNAME, DL_DEPL_MODULE_NAME, ENTERPRISE_ID, DL_PARTITION_ID )
;
