# Ssh.pm: Class Used for Remote Access with ssh

package RDA::Driver::Ssh;

# $Id: Ssh.pm,v 1.11 2012/05/20 20:29:37 mschenke Exp $
# ARCS: $Header: /home/cvs/cvs/RDA_4/src/scripting/lib/RDA/Driver/Ssh.pm,v 1.11 2012/05/20 20:29:37 mschenke Exp $
#
# Change History
# 20120519  MSC  Improve the quoting.

=head1 NAME

RDA::Driver::Ssh - Class Used for Remote Access using ssh

=head1 SYNOPSIS

require RDA::Driver::Ssh;

=head1 DESCRIPTION

The objects of the C<RDA::Driver::Ssh> class are used for execution remote
access requests using F<ssh>.
...
my %tb_cmd = (
  scp => '-BCpq -o ConnectTimeout=30',
  ssh => '-Cnq -o ConnectTimeout=30',
  SCP => '-BCpq',
  SSH => '-Cnq',
  );
...
=head2 S<$h = RDA::Driver::Ssh-E<gt>new($agent)>
...
=item S<    B<'-ssh'> > SSH agent indicator (S)
...
{ my ($cls, $agt, $lim) = @_;
  ...
  unless ($agt->get_setting('REMOTE_SSH_COMMAND') &&
          $agt->get_setting('REMOTE_SCP_COMMAND'))
  { my ($alt, $key, $opt, $scp, $ssh, $val);

    # Determine the remote shell and file copy commands
    return undef
      unless ($ssh = RDA::Object::Rda->find('ssh')) &&
             ($scp = RDA::Object::Rda->find('scp'));

    # Determine the remote shell command
    $opt = $tb_cmd{'ssh'};
    $key = eval "`$ssh $opt -o bad_option=bad_value localhost date 2>&1`";
    $opt = $tb_cmd{'SSH'} if ($alt = ($key =~ m/ConnectTimeout/));
    $agt->set_setting('REMOTE_SSH_COMMAND', $ssh, 'F', 'SSH command');
    $agt->set_setting('REMOTE_SSH_OPTIONS', $opt, 'T', 'SSH options');

    # Determine the remote file copy command
    $opt = $tb_cmd{$alt ? 'SCP' : 'scp'};
    $agt->set_setting('REMOTE_SCP_COMMAND', $scp, 'F', 'SCP command');
    $agt->set_setting('REMOTE_SCP_OPTIONS', $opt, 'T', 'SCP options');
  }
...
{ my ($slf, $var) = @_;
  ...
  # Set an authentication agent when appropriate
  $slf->{'-ssh'} = $slf->{'-ses'}->set_agent unless $slf->{'-ssh'};

  # Determine the command and its options
  $agt = $slf->{'-agt'};
  $nod = $slf->{'-nod'};
  $cmd = $agt->get_setting("REMOTE_$nod\_SSH_COMMAND") ||
    $agt->get_setting('REMOTE_SSH_COMMAND', 'ssh');
  $str = $agt->get_setting("REMOTE_$nod\_SSH_OPTIONS");
  $str = $agt->get_setting('REMOTE_SSH_OPTIONS', $tb_cmd{'ssh'})
    unless defined($str);
  $cmd .= ' '.$str;
...
{ my ($slf, $rec, $dst, $src) = @_;
  ...
  $agt = $slf->{'-agt'};
  $nod = $slf->{'-nod'};
  $cmd = $agt->get_setting("REMOTE_$nod\_SCP_COMMAND") ||
    $agt->get_setting('REMOTE_SCP_COMMAND', 'scp');
  $opt = $agt->get_setting("REMOTE_$nod\_SCP_OPTIONS");
  $opt = $agt->get_setting('REMOTE_SCP_OPTIONS', $tb_cmd{'scp'})
    unless defined($opt);
