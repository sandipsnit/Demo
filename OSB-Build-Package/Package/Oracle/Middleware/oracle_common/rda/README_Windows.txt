...
RUNNING RDA
===========
Before you begin: Log on as the user that owns the Oracle installation. On
some systems, this user will not have the necessary permissions to run all of
the commands and utilities called by RDA (e.g. WinMsd utility, MSInfo32 on
Windows 2000, Windows 2003) If you are running RDA to assist in resolving a
Service Request, the analyst will most likely need the information pertaining
to the Oracle owner.
...
1) Choose the RDA command that you will be using depending on your environment:
   ...
   -- rda.cmd Use this command if Perl is not available. Specifying the
    ...
    The RDA command rda.cmd, rda.pl or perl rda.pl you choose is represented
    as <rda> in the rest of this procedure.

2) The data collection requires an initial setup to determine which information
   should be collected. Enter the following command from the Windows Command or
   DOS prompt to initiate the set up:

   REMEMBER: <rda> means use either rda.cmd, rda.pl or perl rda.pl.
...
5) The final output is also packaged in an archive located in the
   <output_directory> chosen during RDA setup -- the packaged output file will
   have a .zip extension. If the data collection was generated to assist in
   resolving a Service Request, send the report archive (note: file name
   varies, but ends in .zip; e.g. RDA.RDA_systemxyz.zip) to Oracle Support
   by uploading the file via My Oracle Support.

   IMPORTANT:
   When the 'zip' command is found in the path, the reports will be packaged by
   RDA. If the archive is missing, please archive all the files in the
   <output_directory> manually using a tool such as WinZip, PKZip and send
   it to Oracle Support. If ftp'ing the file, please be sure to ftp in BINARY
   format. Do not rename the file, as the file name helps Oracle Support
   quickly identify that RDA output is attached to the service request.
...
PLATFORMS SUPPORTED
===================
At this time, the scripts are supported on the following Microsoft Windows
platforms:
...
=====================
If you receive a "Command not found" error, ensure that you have privileges to
execute sqlplus and Windows System root directory (WINDOWS or WINNT).
...
2) Use an editor such as 'wordpad' to edit the setup file (setup.cfg by
   default).
